...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        for item in self.operands:
            if type(item).__name__ == "Literal":
                nums.append(item.eval(env))
            elif type(item).__name__ == "Name":
                nums.append(env[item.var_name])
            elif type(item).__name__ == "CallExpr":
                nums.append(item.eval(env))
        return env[self.operator.var_name].apply(nums)
