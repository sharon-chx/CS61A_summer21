...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    results = [paragraph for paragraph in paragraphs if select(paragraph)]
...
def accuracy(typed, reference):
    ...
    if denominator == 0 and len(reference_words) == 0:
    	return 100.0
    elif denominator == 0:
    	return 0.0
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word  in valid_words:
    	return typed_word
    result = min(valid_words, key= lambda x: diff_function(typed_word, x, limit))
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if start == goal:            #base case 1: start and goal is the same
    	return 0
    elif limit == 0:             #base case 2: limit has reached, add additional 1 to returned value
    	return 1
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    time_taken = [[times[i]-times[i-1] for i in range(1, len(times))] for times in times_per_player]
    return game(words, time_taken)
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    result = {p_i:[] for p_i in player_indices}
    for w_i in word_indices:
        times = [time(game, p_i, w_i) for p_i in player_indices]
        winner_index = times.index(min(times))
        result[winner_index].append(word_at(game, w_i))
    return [result[p_i] for p_i in player_indices]
