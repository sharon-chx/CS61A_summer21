"""CS 61A presents Ants Vs. SomeBees. Link here: https://cs61a.org/proj/ants/"""
...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        """
        self.name = name
        self.exit = exit      #to the left
        self.bees = []        # A list of Bees
        self.ant = None       # An Ant
        self.entrance = None  # A Place, to the right
...
class Ant(Insect):
    ...
    def remove_from(self, place):
        if type(self).name == "Queen" and self.queen:
        	return
        else:
    ...
    def buff(self):
        ...
        if self.is_container and self.ant_contained and not self.ant_contained.buffed:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        if self.place.bees:
        	return choose_bee(self.place.bees)
        else:
        	cur_place = self.place.entrance
        	cur_range = 1
        	while cur_place:
        		if cur_range >= self.min_range and cur_range <= self.max_range:
        			if not cur_place.is_hive and cur_place.bees:
        				return choose_bee(cur_place.bees)
        		cur_range += 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        Ant.reduce_health(self, damage_amount)
...
class WallAnt(Ant):
	"""WallAnt adds some protection to our glorious home base"""
...
class HungryAnt(Ant):
	"""Hungry Ant select a random Bee from its place and eat it whole. After eating a Bee, 
	a HungryAnt must spend 3 turns chewing before eating again."""
	...
	def action(self, gamestate):
		if self.chew_countdown > 0:
			...
			if self.place.bees:
				chosen_bee = choose_bee(self.place.bees)
...
class TankAnt(ContainerAnt):
	"""TankAnt is a container that protects an ant in its place 
...
# END Problem EC
    ...
    def reduce_health(self, amount):
        ...
        if self.queen and self.health <= 0: 
        	bees_win()
